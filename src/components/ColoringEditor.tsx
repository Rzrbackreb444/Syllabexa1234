...
import { 
  ...
  Terminal, Loader2, Cpu
} from 'lucide-react';
import { useToast } from '../lib/ToastContext';

const LOCAL_STORAGE_KEY = 'syllabexa-coloring-project-v6-apex';
...
export default function ColoringStudio() {
  ...
  const templatesByCategory = useMemo(() => {
    return COLORING_TEMPLATES.reduce((acc, tpl) => {
  ...
  const handleAddPage = () => {
    ...
    setProject(prev => ({ ...prev, pages: [...prev.pages, newPage], selectedPageId: newPage.id }));
    setUndoStack([]);
  ...
  const handleGenerateAI = async () => {
    ...
    try {
      ...
      if (data.svgContent) {
        ...
        setPrompt('');
        setUndoStack([]);
        showToast('AI Vector Canvas Generated!', 'success');
      } else {
  ...
  const handleSvgElementClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (currentSvg) {
      setUndoStack(prev => [...prev.slice(-19), currentSvg]); 
    }
  ...
  const handleUndo = () => {
    ...
    setProject(prev => ({
      ...prev, pages: prev.pages.map(p => p.id === activePage.id ? { ...p, svgContent: previousSvg } : p)
    }));
    ...
    <div ref={studioRef} className="flex-1 bg-[#040506] flex flex-col lg:flex-row h-full font-sans overflow-hidden select-none relative z-0">
      ...
      <style>
        {`
          ...
          .coloring-interactive svg { cursor: var(--cursor-svg) 12 12, crosshair !important; }
      ...
      <motion.aside initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="w-full lg:w-72 bg-[#0c0e12] border-r border-white/5 flex flex-col shrink-0 lg:h-full h-[250px] shadow-2xl z-20">
        <div className="p-5 border-b border-white/5 bg-[#08090c] flex justify-between items-center sticky top-0 z-10">
          ...
          <button onClick={handleAddPage} className="p-1.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 rounded-md transition-colors" title="Add Page">
            <Plus size={16} />
          </button>
        </div>
            ...
            {project.pages.map((p, idx) => {
              ...
              return (
                <motion.div 
                  ...
                  key={p.id} onClick={() => handlePageSelect(p.id)}
                  className={`group flex items-center justify-between p-3.5 rounded-xl cursor-pointer text-xs font-mono transition-all border ${
                    isActive ? 'bg-amber-500/10 border-amber-500/40 text-amber-300 shadow-[0_0_15px_rgba(245,158,11,0.1)] font-bold' 
                ...
                >
                  ...
                  <button onClick={(e) => { e.stopPropagation(); handleDeletePage(p.id); }} className="opacity-0 group-hover:opacity-100 transition-opacity text-slate-500 hover:text-rose-400 p-1">
                    <Trash2 size={14} />
                  </button>
                </motion.div>
              );
        ...
        <div className="p-5 border-t border-white/5 bg-[#08090c] sticky bottom-0 space-y-2">
          <button onClick={handleExportPDF} disabled={isExporting} className="w-full py-3.5 bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-black font-mono text-[11px] font-black uppercase tracking-wider rounded-xl transition-all cursor-pointer shadow-[0_0_15px_rgba(245,158,11,0.2)] flex items-center justify-center gap-2">
          ...
          <button onClick={handleExportPNG} className="w-full py-3.5 bg-black/40 hover:bg-white/5 border border-white/10 text-slate-300 font-mono text-[11px] font-black uppercase tracking-wider rounded-xl transition-all flex items-center justify-center gap-2">
            <Download size={16} /> <span>Download 300 DPI PNG</span>
          </button>
        </div>
      </motion.aside>
      ...
      <main className="flex-1 flex flex-col drafting-table-bg lg:h-full h-[600px] p-6 overflow-hidden relative items-center justify-center">
        ...
        <motion.div initial={{ y: -50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="absolute top-6 left-6 right-6 flex flex-col sm:flex-row gap-3 items-center justify-between bg-[#0c0e12]/90 border border-white/10 p-4 rounded-2xl shadow-2xl z-10 max-w-4xl mx-auto w-full backdrop-blur-md">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <input 
              ...
              className="flex-1 sm:w-80 bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-slate-200 focus:border-amber-500/50 focus:outline-none transition-colors shadow-inner disabled:opacity-50"
            ...
            >
              ...
              <span className="hidden sm:inline">{isGenerating ? 'Synthesizing...' : 'Generate AI'}</span>
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
            ...
            <button onClick={toggleFullScreen} className="p-2.5 bg-black/30 hover:bg-white/10 border border-white/10 text-slate-400 hover:text-white rounded-xl transition-colors cursor-pointer" title="Toggle Fullscreen">
              <Maximize2 size={16} />
            </button>
          </div>
        </motion.div>
              ...
              <div className="flex gap-2">
                {PREMIUM_PALETTE.map(color => (
                  <button
                    ...
                    className={`w-7 h-7 rounded-full border-2 transition-all cursor-pointer shadow-inner ${selectedColor === color ? 'border-amber-400 scale-125 ring-2 ring-amber-500/30' : 'border-white/10 hover:scale-110'}`}
                    style={{ backgroundColor: color }} title={color === '#ffffff' ? 'Eraser (White)' : 'Paint Color'}
                  />
                ))}
              </div>
              <div className="h-6 w-px bg-white/10 mx-2" />
              <button onClick={handleUndo} disabled={undoStack.length === 0} className="text-slate-400 hover:text-amber-400 disabled:opacity-30 disabled:hover:text-slate-400 transition-colors" title="Undo stroke">
                <Undo size={16} />
              </button>
            </motion.div>
          )}
        ...
        <div className="absolute bottom-6 right-6 bg-[#0c0e12]/90 backdrop-blur-md border border-white/10 p-1.5 rounded-2xl flex items-center gap-1 z-10 shadow-2xl">
          <button onClick={() => setZoomLevel(prev => Math.max(50, prev - 10))} className="p-2 text-slate-400 hover:text-white hover:bg-white/10 rounded-xl transition-colors">
            <ZoomOut size={14} />
          </button>
          <span className="px-2 font-mono text-[10px] text-slate-300 font-bold w-12 text-center">{zoomLevel}%</span>
          <button onClick={() => setZoomLevel(prev => Math.min(250, prev + 10))} className="p-2 text-slate-400 hover:text-white hover:bg-white/10 rounded-xl transition-colors">
            <ZoomIn size={14} />
          </button>
        </div>
        ...
        <div className="mt-28 flex-1 flex flex-col items-center justify-center p-4 w-full h-full overflow-hidden">
          <motion.div layout
            ...
            style={{ width: 'min(100%, 550px)', aspectRatio: '1/1', transform: `scale(${zoomLevel / 100})` }}
      ...
      <motion.aside initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="w-full lg:w-80 bg-[#0c0e12] border-l border-white/5 flex flex-col shrink-0 lg:h-full h-[300px] overflow-hidden shadow-2xl z-20">
        <div className="p-5 border-b border-white/5 bg-[#08090c] sticky top-0 z-10 shrink-0">
          <h2 className="text-[11px] font-mono font-bold uppercase tracking-widest text-slate-200 flex items-center gap-2">
            <Palette size={16} className="text-amber-400" /> Professional Templates
          </h2>
        </div>
          ...
          {Object.entries(templatesByCategory).map(([category, templates]) => (
            <div key={category}>
              <h3 className="text-[9px] font-mono font-bold uppercase tracking-widest text-slate-500 mb-3 flex items-center gap-2">
                <span className="w-2 h-px bg-slate-700" /> {category} <span className="flex-1 h-px bg-slate-800" />
              </h3>
              
              <div className="grid grid-cols-2 gap-4">
                {templates.map((tpl, i) => (
                  <div 
                    key={i} onClick={() => loadTemplate(tpl.svgContent, tpl.name)}
                    className="bg-black/30 rounded-2xl p-3 border border-white/5 cursor-pointer hover:border-amber-500/40 hover:bg-amber-500/5 hover:shadow-[0_0_15px_rgba(245,158,11,0.1)] transition-all flex flex-col items-center gap-3 group"
                  >
                    <div 
                    ...
                    />
                    <span className="text-[10px] font-mono font-semibold text-slate-400 group-hover:text-amber-400 text-center truncate w-full">
                      {tpl.name}
                    </span>
                  </div>
                ))}
