...
// Add Google Workspace scopes (Tasks, Drive, Sheets, Forms, Classroom)
  ...
  return onAuthStateChanged(auth, async (user: User | null) => {
    if (user) {
      if (cachedAccessToken) {
        ...
        // If we have a user but no cached token, we can ask for login
        // because we need the fresh accessToken which is only in-memory.
